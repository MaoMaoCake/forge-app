...
import { Alert, Button, Field, Input, Switch, TextArea, useStyles2 } from '@grafana/ui';
import { v4 as uuidv4 } from 'uuid';

// This page is a dedicated "Create Collector" screen that
// uses the createOrUpdateAgent POST /collector endpoint.
// It generates a new UUID client-side so it cannot be edited.
...
function PageCreate() {
  ...
  const [form, setForm] = React.useState<AgentPayload>(() => ({
    agent_uuid: uuidv4(),
    name: '',
    advanced: false,
    agent_config: {
      config: '',
      collectUnixLogs: true,
      collectUnixNodeMetrics: true,
      collectWinLogs: false,
      collectWinNodeMetrics: false,
      collectCadvisorMetrics: true,
  ...
  const handleSubmit = async (evt: React.FormEvent) => {
    ...
    try {
      ...
      // Generate a new UUID for the next collector
      setForm((prev) => ({
        ...prev,
        agent_uuid: uuidv4(),
        name: '',
        agent_config: {
          ...prev.agent_config,
          config: prev.agent_config.config,
        },
      }));
    } catch (err: any) {
      setError(err?.message ?? 'Failed to create collector');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <PluginPage>
      <div className={s.container}>
        ...
        <p>A new UUID is generated for each collector. You cannot edit the UUID.</p>
        ...
        <form className={s.form} onSubmit={handleSubmit}>
          <Field
            label="Agent UUID"
            description="Automatically generated unique identifier for this collector."
          >
